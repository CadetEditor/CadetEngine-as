package
{
	...
	public class C2D_Tests_Transform_01 extends Sprite
	{
		...
		public function C2D_Tests_Transform_01()
		{
			...
			rectangleEntity = createRectangleEntity(cadetScene, 100, 0, 100, 100);
		}
		...
		private function inputDownHandler( event:InputProcessEvent ):void
		{
			if ( event.name == "SPACE" ) {
				if ( count == 0 ) {
					rectangleEntity.parentComponent.children.removeItem(rectangleEntity);
				} else if ( count == 1 ) {
					topRectEntity.children.addItem(rectangleEntity);
				}
				count ++;
			}
		}
